...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, CLIoU=False, eps=1e-7):
    """
    ...
    # IoU
    iou = inter / union
    if CIoU or DIoU or GIoU or CLIoU:
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height
        if CIoU or DIoU or CLIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CLIoU:               ##20231114添加 对角线损失
                v = (4 / math.pi ** 2) * (torch.atan(w2 / h2) - torch.atan(w1 / h1)).pow(2)
                d1_2 = w1 ** 2 + h1 ** 2
                d2_2 = w2 ** 2 + h2 ** 2
                rho2_1 = (d1_2.sqrt() - d2_2.sqrt()) ** 2
                with torch.no_grad():
                    alpha = v / (v - iou + (1 + eps))
                return iou - (rho2 / c2 + rho2_1 / c2 + v * alpha)  # CIoU
...
def plot_pr_curve(px, py, ap, save_dir=Path('pr_curve.png'), names=(), on_plot=None):
    ...
    if 0 < len(names) < 21:  # display per-class legend if < 21 classes
        for i, y in enumerate(py.T):
            ...
            pr_dict[names[i]] = y.tolist()  # 20231217添加 lwd edit
    else:
